...
pub async fn handle_command(
    ...
    match cmd {
        Command::Start => {
            if let Err(e) = bot
                .send_message(msg.chat.id, "🔄 Syncing Enterprise UI...")
        ...
        Command::Remove(ref w) => {
            ...
            if let Some(mut subs) = state.get_mut(&c) {
                subs.remove(&chat_id);
            }
                        ...
                        if let Ok(addr) = Address::try_from(w.as_str()) {
                            if let Ok(utxos) = rpc.get_utxos_by_addresses(vec![addr]).await {
                                let coinbase_utxos: Vec<_> = utxos
                                    ...
                                    .collect();
                                let mut blocks_1h = 0;
                                let mut blocks_24h = 0;
                                let mut blocks_7d = 0;
                                for u in &coinbase_utxos {
                                    let age_in_blocks =
                                        current_daa.saturating_sub(u.utxo_entry.block_daa_score);
                                    if age_in_blocks <= 3600 {
                                        blocks_1h += 1;
                                    }
                                    if age_in_blocks <= 86400 {
                                        blocks_24h += 1;
                                    }
                                    if age_in_blocks <= 604800 {
                                        blocks_7d += 1;
                                    }
                                }
                                let hr_1h = net_hashrate * (blocks_1h as f64 / 3600.0);
                                let hr_24h = net_hashrate * (blocks_24h as f64 / 86400.0);
                                let hr_7d = net_hashrate * (blocks_7d as f64 / 604800.0);
        ...
        Command::Network => {
            ...
            if let Ok(peers) = rpc.get_connected_peer_info().await {
                text.push_str(&format!(
                    "├ <b>Connected Peers:</b> {}\n",
                    peers.peer_info.len()
                ));
            }
            if let Ok(sync) = rpc.get_sync_status().await {
                text.push_str(&format!(
                    "└ <b>Sync Status:</b> {}\n\n",
                    if sync {
                        "100% Synced ✅"
            ...
            if let Ok(hashrate) = rpc.estimate_network_hashes_per_second(1000, None).await {
                text.push_str(&format!(
                    "├ <b>Hashrate:</b> {}\n",
            ...
            match rpc
                .get_block_template(
                    Address::try_from(
                        "kaspa:qq2avyvncscg5dtsk8u4uwjhlr3799dhaqj8k9y6q5y9hpwfxjy6u00pep7vg",
                    )
                    .unwrap(),
                    vec![],
                )
            ...
            {
                Ok(template) => {
                    text.push_str(&format!("├ <b>Status:</b> Solo Ready ✅\n├ <b>Mempool TXs:</b> {}\n└ <b>Next Block Bits:</b> {}\n", template.block.transactions.len(), template.block.header.bits));
                }
                Err(_) => {
                    text.push_str("└ <b>Status:</b> Not Ready ❌\n");
                }
            }
        ...
        Command::Stats => {
            if is_admin {
                let total_users: HashSet<i64> =
                    state.iter().flat_map(|e| e.value().clone()).collect();
                let ping_start = tokio::time::Instant::now();
                let node_status = match rpc.get_server_info().await {
                    Ok(_) => format!("Online 🟢 ({}ms)", ping_start.elapsed().as_millis()),
